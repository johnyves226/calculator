...
const convertClipboardEvent = (event) => ({
    ...convertEvent(event),
    clipboardData: convertDataTransferObject(event.clipboardData),
});
const convertCompositionEvent = (event) => ({
    ...convertUiEvent(event),
    data: event.data,
});
const convertInputEvent = (event) => ({
    ...convertUiEvent(event),
    data: event.data,
...
const convertKeyboardEvent = (event) => ({
    ...convertUiEvent(event),
    code: event.code,
    isComposing: event.isComposing,
    altKey: event.altKey,
...
const convertUiEvent = (event) => ({
    ...convertEvent(event),
    detail: event.detail,
});
const convertAnimationEvent = (event) => ({
    ...convertEvent(event),
    animationName: event.animationName,
    pseudoElement: event.pseudoElement,
    elapsedTime: event.elapsedTime,
});
const convertTransitionEvent = (event) => ({
    ...convertEvent(event),
    propertyName: event.propertyName,
    pseudoElement: event.pseudoElement,
    elapsedTime: event.elapsedTime,
});
const convertFocusEvent = (event) => ({
    ...convertUiEvent(event),
    relatedTarget: convertElement(event.relatedTarget),
});
const convertDeviceOrientationEvent = (event) => ({
    ...convertEvent(event),
...
const convertDragEvent = (event) => ({
    ...convertMouseEvent(event),
    dataTransfer: convertDataTransferObject(event.dataTransfer),
});
const convertGamepadEvent = (event) => ({
    ...convertEvent(event),
    gamepad: convertGamepad(event.gamepad),
});
const convertPointerEvent = (event) => ({
    ...convertMouseEvent(event),
...
const convertSubmitEvent = (event) => ({
    ...convertEvent(event),
...
const convertGenericElement = (element) => ({
    tagName: element.tagName,
    boundingClientRect: { ...element.getBoundingClientRect() },
});
const convertMediaElement = (element) => ({
...
const elementConverters = {
    ...
    BUTTON: (element) => ({ value: element.value }),
    ...
    METER: (element) => ({ value: element.value }),
    OPTION: (element) => ({ value: element.value }),
    OUTPUT: (element) => ({ value: element.value }),
    PROGRESS: (element) => ({ value: element.value }),
    SELECT: (element) => ({ value: element.value }),
    TEXTAREA: (element) => ({ value: element.value }),
    VIDEO: convertMediaElement,
};
const convertGamepad = (gamepad) => ({
    ...
    hapticActuators: Array.from(gamepad.hapticActuators).map(convertGamepadHapticActuator),
});
const convertGamepadButton = (button) => ({
    pressed: button.pressed,
    touched: button.touched,
    value: button.value,
});
const convertGamepadHapticActuator = (actuator) => ({
    type: actuator.type,
});
...
function convertDataTransferObject(dataTransfer) {
    if (!dataTransfer) {
        return null;
    }
...
function convertSelection(selection) {
    ...
    const { type, anchorNode, anchorOffset, focusNode, focusOffset, isCollapsed, rangeCount, } = selection;
    if (type === "None") {
        return null;
    }
    return {
...
function convertTouch({ identifier, pageX, pageY, screenX, screenY, clientX, clientY, force, radiusX, radiusY, rotationAngle, target, }) {
    return {
        identifier,
        pageX,
        pageY,
...
async function loadImportSource(vdomImportSource, client) {
    let module;
...
function createChildren(model, createChild) {
    if (!model.children) {
        return [];
    }
    else {
...
function createAttributes(model, client) {
    ...
    return [
        ...
        function (...args) {
            const data = Array.from(args).map((value) => {
                ...
                return convert(event.nativeEvent);
            });
...
function Layout(props) {
    const currentModel = reactExports.useState({ tagName: "" })[0];
    const forceUpdate = useForceUpdate();
    reactExports.useEffect(() => props.client.onMessage("layout-update", ({ path, model }) => {
        if (path === "") {
...
function StandardElement({ model }) {
    ...
    return reactExports.createElement(model.tagName === "" ? reactExports.Fragment : model.tagName, createAttributes(model, client), ...createChildren(model, (child) => {
        return React.createElement(Element, { model: child, key: child.key });
    }));
}
function UserInputElement({ model }) {
    const client = reactExports.useContext(ClientContext);
    ...
    // having keys. Warning about this must now be the responsibility of the client
    // providing the models instead of the client rendering them.
    return reactExports.createElement(model.tagName,
    // overwrite
...
var aa=reactExports,m=requireObjectAssign(),r=requireScheduler();function y(a){for(var b="https://reactjs.org/docs/error-decoder.html?invariant="+a,c=1;c<arguments.length;c++)b+="&args[]="+encodeURIComponent(arguments[c]);return "Minified React error #"+a+"; visit "+b+" for the full message or use the non-minified dev environment for full errors and additional helpful warnings."}if(!aa)throw Error(y(227));var ba=new Set,ca={};function da(a,b){ea(a,b);ea(a+"Capture",b);}
	...
	"children dangerouslySetInnerHTML defaultValue defaultChecked innerHTML suppressContentEditableWarning suppressHydrationWarning css".split(" ").forEach(function(a){D[a]=new B(a,0,!1,a,null,!1,!1);});[["acceptCharset","accept-charset"],["className","class"],["htmlFor","for"],["httpEquiv","http-equiv"]].forEach(function(a){var b=a[0];D[b]=new B(b,1,!1,a[1],null,!1,!1);});["contentEditable","draggable","spellCheck","value"].forEach(function(a){D[a]=new B(a,2,!1,a.toLowerCase(),null,!1,!1);});
...
function mount(element, client) {
    reactDomExports.render(React.createElement(Layout, { client: client }), element);
}
...
function createReconnectingWebSocket(props) {
    ...
    const connect = () => {
        ...
        socket.current.onclose = () => {
            ...
            console.info(`ReactPy reconnecting in ${(interval / 1000).toPrecision(4)} seconds...`);
...
function nextInterval(currentInterval, backoffMultiplier, maxInterval) {
...
class ReactPyDjangoClient extends BaseReactPyClient {
    constructor(props) {
        super();
        this.urls = props.urls;
        this.socket = createReconnectingWebSocket(Object.assign({ readyPromise: this.ready, url: this.urls.componentUrl, onMessage: async ({ data }) => this.handleIncoming(JSON.parse(data)) }, props.reconnectOptions));
    }
    sendMessage(message) {
        var _a;
        (_a = this.socket.current) === null || _a === void 0 ? void 0 : _a.send(JSON.stringify(message));
    }
...
function mountComponent(mountElement, host, urlPrefix, componentPath, resolvedJsModulesPath, reconnectStartInterval, reconnectMaxInterval, reconnectMaxRetries, reconnectBackoffMultiplier) {
    // Protocols
